...
def psi_hartmannneff_C(C_, param): # paper german benchmarks Archives, 2021
    ...
    if(C_.ufl_shape[0] == 2):    
        C = ufl.as_tensor([[C_[0,0], C_[0,1], 0], [C_[1,0], C_[1,1], 0], [0, 0, 1]])
    else:
        C = C_
    ...
    Cbar = J**(-2/3)*C
    I1 = ufl.tr(Cbar)
    I2 = ufl.tr(ufl.cofac(Cbar))
    # I2 = 0.5*(ufl.tr(Cbar)**2 - ufl.tr(Cbar*Cbar))
...
# strain_ should be a ufl.variable, isomorph to strain 
def get_stress_tang_from_psi(psi, strain, strain_): 
    stress = ufl.diff(psi(strain), strain_)
    tang = ufl.diff(stress, strain_)
    return stress, tang
